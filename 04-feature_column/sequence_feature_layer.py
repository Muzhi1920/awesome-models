...
class SequenceFeatures(kfc._BaseFeaturesLayer):
  ...
  def _target_shape(self, input_shape, total_elements):
    print('target_shape',input_shape[0], input_shape[1], total_elements)
  ...
  def call(self, features, training=None):
    ...
    for column in self._feature_columns:
      with backend.name_scope(column.name):
        ...
        output_tensors[column.name] = self._process_dense_tensor(column, dense_tensor)
        sequence_lengths[column.name] = sequence_length
